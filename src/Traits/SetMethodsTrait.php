...
trait SetMethodsTrait {
    ...
    function sdiffstore($dest, array $keys) {
		...
		return $this->exec( $this->protocol( __METHOD__, $dest, $keys ) );
    }
    ...
    function smove($source, $dest, $member) {
		return $this->exec( $this->protocol( __METHOD__, $source, $dest, $member ) );
    }
    ...
    function spop($key, $count = 1) {
		return $this->exec( $this->protocol( __METHOD__, $key, $count ) );
    }
    ...
    function sscan($key, $cursor, $pattern = "", $count = "") {
		...
		return $this->exec( $this->protocol( __METHOD__, $args ) );
    }
